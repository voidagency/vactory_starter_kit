...
class TermNormalizationService
{
    ...
    protected function validateTerm($value, $field_config, $line_number, array &$term_values, array &$errors)
    {
        ...
        if (!isset($term_values[$field_config])) {
            ...
            'stripped_originals' => [$stripped_value],  // Add stripped version
            'variations' => [],
        ...
        if (!empty($differences)) {
            ...
            $term_values[$field_config]['stripped_originals'][] = $stripped_value;  // Don't forget to add stripped version
    ...
    protected function hasWordBoundaryDifference($term1, $term2): bool
    {
        ...
        // If one has more words than the other, check if they form the same term when joined
    ...
    protected function hasMissingCharacterDifference($term1, $term2): bool
    {
        ...
        // If the number of words is different, it's not a missing character difference && // Multi-word case
        if (count($words1) !== count($words2)) {
            return false;
        }
    ...
    protected function isMissingCharacterInWord($word1, $word2): bool
    {
        ...
        // If the lengths differ by more than 1, it's not a missing character difference
        if (abs($len1 - $len2) !== 1) {
            return false;
        }
        ...
        // Check if the shorter word can be formed by removing one character from the longer word
    ...
    protected function formatErrorMessages(array $errors)
    {
        ...
        foreach ($errors as $field => $data) {
            $message = $this->t(
                'Different variations found for field "@field":', [
                '@field' => $field,
                ]
            ) . '<br/>';
            ...
            foreach ($data['variations'] as $variation) {
                ...
                if (!empty($variation['differences'])) {
                    ...
                    $message .= ' (' . implode(', ', $variation['differences']) . ')';
                }
