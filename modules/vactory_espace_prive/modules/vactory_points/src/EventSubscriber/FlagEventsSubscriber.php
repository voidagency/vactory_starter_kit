...
class FlagEventsSubscriber implements EventSubscriberInterface {
  ...
  public function onFlag(FlaggingEvent $event) {
    ...
    $rules = array_filter($rules, function ($rule) use ($flag_id) {
      return in_array($flag_id, $rule['action']['concerned_flags']) && $rule['action']['value'] === 'flag';
    });
  ...
  public function onUnflag(UnflaggingEvent $event) {
    ...
    $entity = $flagging->getFlaggable();
    $flag_id = $flagging->getFlagId();
    $action = 'unflag/' . $flag_id;
    $rules = $config->get('rules');
    $rules = array_filter($rules, function ($rule) use ($flag_id) {
      return in_array($flag_id, $rule['action']['concerned_flags']) && $rule['action']['value'] === 'unflag';
    });
    if (!empty($rules)) {
      $this->vactoryPointsManager->triggerUserPointsUpdate($action, $entity);
    }
