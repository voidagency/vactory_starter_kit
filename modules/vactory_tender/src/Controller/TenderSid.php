...
class TenderSid extends ControllerBase {
   ...
   * {@inheritDoc}
   */
  public function __construct(EntityTypeManagerInterface $entityTypeManager, StreamWrapperManagerInterface $streamWrapperManager, VactoryDevTools $vactoryDevTools) {
  ...
  public function getFileUrl($sid, Request $request) {
    try {
      ...
      $node = $this->entityTypeManager->getStorage('node')
        ->load($data['tender']);
      $mid = $node->get('field_vactory_media_file')->getValue()[0]['target_id'];
      $media = Media::load($mid);
      $fid = $media->getSource()->getSourceFieldValue($media);
      $file = File::load($fid);

      if ($file) {
        $url = $this->streamWrapperManager->getViaUri($file->getFileUri())
          ->getExternalUrl();
