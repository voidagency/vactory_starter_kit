...
class CampaignsService {
  ...
  public function mailchimpGetInfos() {
    ...
    try {
      ...
      $this->loggerChannelFactory->get('bfig_mailchimp')->error('An error occurred requesting Mailchimp information from Vactory mailchimp webform handler. "{message}"', ['message' => $e->getMessage()]);
    }
    return $infos;
  }
  ...
  public function mailchimpCreateCampaign($template, $recipients, $settings) {
    ...
    try {
      ...
      if (!empty($result->id)) {
        ...
        $this->loggerChannelFactory->get('bfig_mailchimp')->notice(t('Campaign %name (%cid) was successfully saved on Mailchimp.',
          ['%name' => $settings->title, '%cid' => $campaign_id]));
        $mc_campaigns->setCampaignContent($campaign_id, $content_parameters);
      }
    }
    catch (\Exception $e) {
      $this->loggerChannelFactory->get('bfig_mailchimp')->error('An error occurred while creating this campaign on mailchimp: {message}', [
  ...
  public function mailchimpSendCampaign($campaign) {
    ...
    try {
      ...
      if (($result->status == MAILCHIMP_STATUS_SENDING) || ($result->status == MAILCHIMP_STATUS_SENT)) {
        ...
        $this->loggerChannelFactory->get('bfig_mailchimp')->notice('Mailchimp campaign {id} has been sent.', [
          'id' => $campaign->id,
        ]);
      ...
      $this->loggerChannelFactory->get('bfig_mailchimp')
  ...
  public function mailchimpSendNewsletter($node, $list_id) {
    ...
    try {
      ...
      $this->loggerChannelFactory->get('bfig_mailchimp')->error($response['message']);
